...
def generate_page(from_path, dest_path, template_path = DEFAULT_TEMPLATE_PATH):
...
def generate_pages_recursive(dir_path_content, dest_dir_path, template_path = DEFAULT_TEMPLATE_PATH):
    ...
    for entry in content_entries:
        ...
        if os.path.isdir(entry_path):
            os.mkdir(entry_dest)
            generate_pages_recursive(entry_path, entry_dest, template_path)
        if os.path.isfile(entry_path) and entry_path.endswith(".md"):
            html_dest = entry_dest.replace(".md", ".html")
            generate_page(entry_path, html_dest, template_path)
